...
public class ThinLensFOV implements Camera {
	...
	public ThinLensFOV(double k, double focalDistance, double lensRadius) {
		this.k = k;
		this.focalDistance = focalDistance / k;
		this.lensRadius = lensRadius;
	}
	
	
	public static ThinLensFOV fov(double phi, double focalDistance, double lensRadius) {
		return new ThinLensFOV(1 / Numeric.tanT(phi / 2), focalDistance, lensRadius);
	}
	...
	public Ray exitingRay(Vector pSensor) {
		...
		Vec3 q = Vec3.zp(k, pSensor).mul(focalDistance);
		return Ray.pq(p, q);
	}
